...
if __name__ == "__main__":
    ...
    r1 = Runner(data, FullLSTMModelOneWay(data.vocab_len, data.max_len, data.feature_len-1), epochs, batch_size, adam_init, **num_instances).get_results_from_model(loss_fn).save_results(results_folder, prefix)
    r1.save_model(build_folder, prefix)
    r2 = Runner(data, SeqToSeqLSTMModelOneWay(data.vocab_len, data.max_len), epochs, batch_size, adam_init, **num_instances).get_results_from_model(loss_fn).save_results(results_folder, prefix)
    r2.save_model(build_folder, prefix)
    r3 = Runner(data, SimpleLSTMModelOneWay(data.vocab_len, data.max_len), epochs, batch_size, adam_init, **num_instances).get_results_from_model(loss_fn).save_results(results_folder, prefix)
    r3.save_model(build_folder, prefix)
    r4 = Runner(data, SimpleLSTMModelTwoWay(data.vocab_len, data.max_len), epochs, batch_size, adam_init, **num_instances).get_results_from_model(loss_fn).save_results(results_folder, prefix)
    ...
    r6 = Runner(data, TransformerModelTwoWay(data.vocab_len, data.max_len), epochs, batch_size, adam_init, **num_instances).get_results_from_model(loss_fn).save_results(results_folder, prefix)
